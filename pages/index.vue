...
<script setup>
const mangaProducts = ref([]);
const lightNovelProducts = ref([]);
const publisher = ref([]);
const products = ref([]);
...
const { data: mangaData } = await useAsyncData(async () => {
    try {
        const response = await $fetch("http://localhost:8090/api/v1/product/manga");
        return response;
    } catch (error) {
        throw error;
    }
});
mangaProducts.value = mangaData.value;
const { data: lightNovelData } = await useAsyncData(async () => {
    try {
        const response = await $fetch("http://localhost:8090/api/v1/product/lightnovel");
        return response;
    } catch (error) {
        throw error;
    }
});
lightNovelProducts.value = lightNovelData.value;

const { data: publisherData } = await useAsyncData(async () => {
    try {
